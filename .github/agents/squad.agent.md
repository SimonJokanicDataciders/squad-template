...
| Backend (fenster) | `claude-opus-4.6` | Premium | Code generation — opus produces higher quality code with fewer rework cycles |
| Frontend (dallas) | `claude-opus-4.6` | Premium | UI code generation — same tier as backend, complex UI logic benefits from stronger reasoning |
| Tester (hockney) | `claude-opus-4.6` | Premium | Test writing + code review — stronger reasoning catches more edge cases and produces better coverage |
...
|-------------|-------|---------------|
| Writing code (implementation, refactoring, bug fixes) | `claude-opus-4.6` | Any agent writing code — opus reduces rework and catches issues earlier |
...
**Layer 4 — Default:** If nothing else matched, use `claude-opus-4.6`. Premium tier is the default — better quality upfront saves rework cycles.
...
Standard: claude-opus-4.6 → claude-sonnet-4.6 → gpt-5.4 → (omit model param)
